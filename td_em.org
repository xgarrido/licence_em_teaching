   ...
   2) Équation locale de conservation de la charge
      ...
      *Remarques*
      ...
      - Dans le conducteur, la densité de courant $\vv{j}$ s'exprime en fonction
        ...
        donc les charges se déplaçant en surface.
...
3) *Moment cinétique* $\vv{\sigma}_O=\vv{r}\times\vv{p}=\vv{OM}\times\vv{p}$ et
   ...
   3) Expression de $\sigma_z$ [fn:2]
      ...
      #+BEGIN_SRC latex
        \begin{align*}
          \text{\ding{193}}&=q\int_0^T\vv{OM}\times\underbrace{\left(\vv{v}\times\vv{B}\right)}_{\vv{v}\parallel\vv{B}=0}.\vv{u}_zdt
        \end{align*}
      #+END_SRC
      ou
      #+BEGIN_SRC latex
...
#+ATTR_LATEX: :float wrap :placement {l}{0.3\textwidth} :width 0.3\textwidth
[[./cylinders.pdf]]
